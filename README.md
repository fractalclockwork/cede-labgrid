...
- [LabGrid](https://labgrid.readthedocs.io/) (>=25.0, gRPC) coordinator runs in Docker on the Dev-Host; **N exporters** connect to one coordinator
- Multi-exporter provisioning: all gateways declared in `lab/config/lab.yaml` under `exporters`; `make lg-render-configs` generates exporter YAMLs, `env/remote.yaml`, `places.yaml`, and `setup_places.sh` from a single source of truth
- Cloud-init bootstraps each exporter at first boot: labgrid venv, systemd service, `/var/cache/labgrid`, sysctl hardening
- Custom drivers: `PicotoolFlashDriver`, `AvrdudeFlashDriver`, `CedeValidationDriver` (ConsoleExpectMixin), `CedeResetDriver`, `CedeI2CDriver`, `FlashProtocol` (ABC)
- `CedeStrategy` (GraphStrategy): `off` → `flashed` → `validated` state machine
- Firmware attestation via `.digest` sidecar files
- `pytest --lg-env env/remote.yaml` replaces Make-based hardware smoke tests
- LabGrid runs alongside the legacy SSH/rsync `pi-gateway-*` Make targets; both pipelines coexist
- End-to-end manual walkthrough: [lab/pi/docs/labgrid-manual-flash.md](lab/pi/docs/labgrid-manual-flash.md)
- Adding a new gateway: [lab/pi/docs/multi-exporter-setup.md](lab/pi/docs/multi-exporter-setup.md)
...
cede_labgrid/             # custom LabGrid drivers, protocols, and strategies
  drivers/
    picotool_flash.py       # PicotoolFlashDriver (UF2 via exporter; FlashProtocol)
    avrdude_flash.py        # AvrdudeFlashDriver (HEX via exporter; FlashProtocol)
    cede_validation.py      # CedeValidationDriver (ConsoleExpectMixin + digest sidecar)
    cede_reset.py           # CedeResetDriver (picotool reboot / DTR toggle)
    cede_i2c.py             # CedeI2CDriver (SSH-based I2C bus operations)
  protocols/
    flash.py                # FlashProtocol ABC
  strategies/
...
env/                      # LabGrid environment configs (generated by make lg-render-configs)
  remote.yaml               # client env: targets, drivers, images, imports (generated)
  exporters/                # per-exporter configs: <name>.yaml (generated, gitignored)
  cede-pi-exporter.yaml    # deprecated -- kept as reference
lab/
  config/
    lab.example.yaml
    lab.schema.json
  ...
  pi/
    ...
    docs/                   # SD card, CLI flash, serial console, LabGrid walkthrough, multi-exporter setup
