#!/usr/bin/env bash
# Run Pi gateway checks and Uno flashing from the dev-host (SSH into cede-pi, optional rsync/scp).
#
# Environment:
#   GATEWAY               — SSH target (default: pi@cede-pi.local)
#   GATEWAY_REPO_ROOT      — Repo path ON the Pi (default: ~/cede)
#   UNO_ONLY              — when syncing deps, omit Pico UF2 script (same as sync_gateway_flash_deps.sh)
#
# Usage (from repo root unless noted):
#   GATEWAY=pi@cede-pi.local lab/pi/scripts/devhost_pi_gateway.sh health
#   lab/pi/scripts/devhost_pi_gateway.sh resolve-port-uno  — print Uno tty resolved ON the gateway
#   lab/pi/scripts/devhost_pi_gateway.sh subtarget-check
#   lab/pi/scripts/devhost_pi_gateway.sh print-serial
#   lab/pi/scripts/devhost_pi_gateway.sh flash-uno --hex lab/uno/hello_lab/build/hello_lab.ino.hex [--port /dev/ttyACM0]
#   ... [--port omitted: gateway auto-picks Arduino / sane ttyUSB / lone non-Pico ACM]
#   lab/pi/scripts/devhost_pi_gateway.sh validate-uno-serial [--port /dev/ttyACM0]
