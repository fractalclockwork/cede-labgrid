"""Shared hello_lab serial attestation (banner prefix + digest= token)."""
...
def banner_prefix_ok(buf: bytes, role: Role) -> bool:
    p = BANNER_PICO_PREFIX if role == "pico" else BANNER_UNO_PREFIX
    return p in buf


def digest_banner_line(buf: bytes, role: Role) -> str | None:
    """First serial line containing the hello_lab prefix and digest= (for human-readable logs)."""
    prefix = "CEDE hello_lab ok" if role == "uno" else "CEDE hello_lab rp2 ok"
...
def attestation_failure_reason(buf: bytes, role: Role, digest_must_equal: str) -> str | None:
    ...
    Always requires hello_lab banner prefix and a digest= token.
