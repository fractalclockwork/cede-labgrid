...
/* CEDE hello_lab (RP2040): USB CDC banner + LED + I2C slave @0x42 (Pi→Pico). On USB 'm', runs Pi-free
 * pico→uno master read of Uno reg0 @0x43 (see lab/pi/docs/bus-wiring.md). */
...
/* Briefly become I2C master on GP0/1 to read Uno hello_lab reg0 (USB trigger 'm'). */
...
int main(void) {
    ...
    sleep_ms(1500);
    printf("CEDE hello_lab rp2 ok digest=%s (i2c 0x42 @ GP0/1; uno lab 0x43; send m for pico→uno I2C test)\n",
           CEDE_IMAGE_ID);
    fflush(stdout);
