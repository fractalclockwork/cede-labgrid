...
embedding keys under the gateway login user, and rejecting invalid inputs.
"""
...
def _set_authorized_keys(cfg: dict, path: str) -> None:
    """Set authorized_keys_file on the first exporter (new format)."""
    if "exporters" in cfg:
        cfg["exporters"][0]["authorized_keys_file"] = path
    else:
        cfg.setdefault("raspberry_pi_bootstrap", {})["authorized_keys_file"] = path


def _set_ssh_user(cfg: dict, user: str) -> None:
    if "exporters" in cfg:
        cfg["exporters"][0]["ssh_user"] = user
    cfg.setdefault("hosts", {}).setdefault("pi", {})["ssh_user"] = user
...
def test_e2e_public_key_embedded_for_pi_user(
    ...
    cfg = copy.deepcopy(base_lab_cfg)
    _set_authorized_keys(cfg, str(pub.resolve()))
...
def test_e2e_missing_authorized_keys_file_rejected(
    ...
    cfg = copy.deepcopy(base_lab_cfg)
    _set_authorized_keys(cfg, str(tmp_path / "nonexistent.pub"))
